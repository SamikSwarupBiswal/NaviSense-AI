...
| [Rishav](rishav/guidance.md) | Risk, voice UX, accessible shell, lifecycle and integration | Verified (automated & device scope) | Master app shell running live on OPPO CPH2753; Voice control for Stop button implemented with speech barge-in, partial-results preemption, and "bus stop" destination guard; CampusDestinationResolver, RouteValidator, and NavigationSnapshot implemented; AB1 defect resolved; 154/154 unit tests pass; APK assembles clean (SHA-256: 407BEC3D...); installed on physical device 6545Q8A6X89TW8ZX (PID 4584) | Supervised outdoor campus walking walkthrough across all 11 POIs |
| [Samik](samik/guidance.md) | Android camera, both inference adapters, tracking, search, and the complete ultrasonic-plus-vision fusion engine | Ready for review (automated scope) | Explicit FusionInput/FusionState/FusionTransition reducer, local PyTorch Lite model activation with asset hash verification, CameraX/USB coordination, exact risk formulas, guidance transitions, and 84 passing JVM tests implemented by Codex for Samik | Connect OPPO phone plus ESP32-S3/HC-SR04; run live detections, device tests, latency measurements, and receiver review |
| [Rohan](rohan/guidance.md) | Hardware, firmware, mount, USB and Android sensor adapter | In progress | Hardware spec, 10Hz ESP32 firmware, serial test tool (20/20 pass), replay harness (10/10 pass), dev.navisense.usb package & UsbSensorAdapter (unit tests pass) | Physical bench ranging across 6 distances (AC-08); benchmark live USB OTG streaming to phone |
| [Subham](subham/guidance.md) | Laptop Locate, Hard Scan, SQLite, API and Android memory client | Delivered & Integrated with Locate YOLO | Tabletop dataset delivered (494 images across SES_01–SES_04, WEARABLE [104 imgs], and SECURITY [116 imgs]); OpenCvFrameSource & YoloLocateDetector wired to HardScanEngine and FastAPI service; run_hard_scan.py CLI runner created; 43/43 laptop tests PASS | Execute live Hard Scan trial with physical laptop camera; test Android MemoryClientContract sync |
| [Spandan](spandan/guidance.md) | Models, datasets, training, exports and quality evaluation | Delivered S09 candidate weights (automated) | Roboflow 255-image dataset ingested & partitioned (179 train, 51 val, 25 test; keys & wallet); trained Locate YOLOv8n (val mAP50 93.7%, test mAP50 95.0%, keys P=90.3% R=84.5%, wallet P=93.7% R=99.0%); exported TorchScript (SHA-256: 6422ac4e...) and PyTorch Lite (SHA-256: 85a6d1cf...); verify_smoke.py 100% PASS; packaged into Android assets | Subham connect locate_smoke.pt to Hard Scan; Samik/Rishav test on OPPO CPH2753 |
...
| 2026-09-16 | Codex for Rishav | Implemented VIT Chennai campus destination resolver, route validator, and snapshot formatting (AB1 defect fix) | Created `CampusDestinationResolver.kt` (all 11 campus POIs, token matching, aliases, disambiguation, off-campus detection), `RouteValidator.kt` (G01–G06 geometry validation and provider-vs-geometry length consistency), updated `PedestrianProgressCalculator.kt` (polyline chainage, along-route projection), `PedestrianNavigationEngine.kt` (accuracy gating <=20m, immutable `NavigationSnapshot` dispatch with unified distance formatting), and `SessionCoordinator.kt`/`MainActivity.kt` (indoor arrival advisory within 25m). 154/154 unit tests PASS (including 6 resolver tests, 4 route validator tests, and 110 campus route pairs). Debug APK assembled (SHA-256 `3F99842063DE91CABC1D7BA7DB8336C334EEF52FFB800AE235074DC054376132`) and installed on OPPO CPH2753 (`6545Q8A6X89TW8ZX`). |
...
### VIT Chennai Campus Navigation & AB1 Defect Remediation — 2026-09-16

Remediated reported AB1 navigation bug (~178 m spoken, 728100 m left and 223 m next turn on screen) and implemented campus-wide navigation across all 11 VIT Chennai POIs per approved plan:
1. **Root Cause Analysis (AB1 Defect)**:
   - Voice command `"Academic Block 1 (AB1)"` was passed directly to unconstrained Android native Geocoder. Geocoder resolved it to a remote location (~728 km away).
   - OSRM computed a 728,100 m route; `PedestrianNavigationEngine.startRoute` announced step 0 (178 m), while UI displayed 728,100 m left and 223 m next turn.
2. **Campus Destination Resolver (C01, C02, C03, C04)**:
   - Created `CampusDestinationResolver.kt` with an offline campus dictionary for all 11 POIs (AB1, AB2, AB3, Main Gate, Admin Block, Library, Gazebo, Food Court, C-Block, D-Block, Sports Complex).
   - Tokenizes and cleans queries, matches aliases (e.g. "ab1", "block 1", "academic block one"), detects ambiguity, and flags off-campus queries (e.g. Phoenix Mall).
   - Routed campus POIs directly to offline graph `vit_chennai_map.json` navigation via `startMapNavigationToPoi`.
3. **Route & Geometry Validator (G01–G06)**:
   - Created `RouteValidator.kt` to validate route coordinates, step distance consistency, endpoint deviation (<= 150m), and provider vs geometry consistency.
   - Throws `InconsistentRouteException` and halts navigation if `|geometryLength - providerTotal| > max(25m, 0.15 * providerTotal)`.
   - Throws `MalformedGeometryException` on invalid or out-of-bounds coordinates.
4. **Along-Route Progress Tracking & Accuracy Gating (C05, C06)**:
   - Updated `PedestrianProgressCalculator.kt` with `PolylineChainage` and `matchFixToRoute` for robust projection onto route segments.
   - Updated `PedestrianNavigationEngine.kt` to enforce GPS accuracy `<= 20m` for arrival confirmation, preventing premature arrival announcements.
   - Dispatches immutable `NavigationSnapshot` containing `totalDistanceMeters`, `remainingDistanceMeters`, `nextManeuverDistanceMeters`, and formatted strings (`X m` or `X.Y km`).
5. **UI & Session Integration (C07)**:
   - Updated `SessionCoordinator.kt` and `MainActivity.kt` to use `CampusDestinationResolver`.
   - Wired `NavigationSnapshot` directly to UI status display.
   - Triggers explicit entrance arrival / indoor navigation limitation alert (`"You have arrived near the entrance. Indoor navigation is not available."`) when within 25m of destination.
6. **Automated Verification**:
   - `CampusDestinationResolverTest`: 6 tests PASS (exact aliases, numbers, abbreviations, off-campus rejection).
   - `RouteValidatorTest`: 4 tests PASS (coherence, geometry mismatch rejection, endpoint bounds).
   - `CampusMapIntegrityTest`: All 11 POIs and 110 directed campus route pairs compute valid non-null routes on `vit_chennai_map.json` (PASS).
...
### Stop Button Voice Control & Docs Gitignore Integration — 2026-09-16

1. **Stop Button Voice Control ("Stop" command)**:
   - Upgraded `VoiceCommandParser.kt` to recognize `"stop"`, `"stop button"`, `"press stop"`, `"click stop button"`, `"emergency stop"`, `"halt"`, `"freeze"`, `"cancel"`, and `"quit"`.
   - Added destination safeguard: explicit exclusion for `"bus stop"` (e.g. "take me to bus stop", "vandalur bus stop") preventing emergency stop interception of campus transit destinations.
   - Enabled speech barge-in and partial-results preemption in `VoiceCommandManager.kt` allowing users to stop the app immediately even during ongoing navigation speech.
   - Connected `onStopRequested` callback in `VoiceDestinationRecognizer.kt` for direct preemption during voice destination input.
   - Consolidated UI/lifecycle stop logic in `MainActivity.kt` (`triggerStopButton`): visual press effect on `btnStop` (`isPressed = true`), immediate speech cancellation via `SpeechArbiter.cancelAll()`, haptic cancellation, camera perception stop, sensor stop, wake lock release, and audible `"Stopped. Idle."` confirmation.
2. **Docs Folder Gitignore**:
   - Added `docs/` to root `.gitignore`.
3. **Verification & Device Run**:
   - 154/154 Android unit tests pass (`.\gradlew.bat :app:testDebugUnitTest`).
   - Debug APK compiled clean: SHA-256 `407BEC3DC6E67C01F0EABD39668D05F154082B0D1489FBC4A3CC0C3DE9636379`.
   - Installed and running on physical OPPO CPH2753 (`6545Q8A6X89TW8ZX`, PID 4584); logcat verified active OnDeviceSpeechRecognizer and CameraX preview streaming.
