...
| [Subham](subham/guidance.md) | Laptop Locate, Hard Scan, SQLite, API and Android memory client | In progress | Tabletop dataset delivered (193 images across SES_01, SES_02, SES_03; 108 keys, 105 wallets, 48 negatives); SQLite persistence engine (10/10 PASS), Hard Scan engine (10/10 PASS), FastAPI REST service & runner (11/11 PASS); 43/43 laptop tests pass | Connect trained Locate YOLO artifact when Spandan delivers S09 candidate |
...
| [Samik](samik/guidance.md) | Android camera, both inference adapters, tracking, search, and the complete ultrasonic-plus-vision fusion engine | In progress | CameraXAnalyzer, CameraTimestampMapper, Yuv420RgbConverter, KEEP_ONLY_LATEST concurrency gating implemented; bound to live PreviewView (30 FPS) on OPPO CPH2753; user explicitly assigned the multimodal fusion engine to Samik on 2026-09-15 | Implement the deterministic fusion reducer, exact timing/hold rules, and integration tests |
| [Rishav](rishav/guidance.md) | Risk, voice UX, accessible shell, lifecycle and integration | In progress | Master app shell v0 running live on OPPO CPH2753 (Android 16); CameraX live preview window rendering at 30 FPS; AndroidUsbCdcTransport & UsbSensorAdapter wired to SessionCoordinator; haptics & TalkBack integrated; 78/78 tests pass | End-to-end walking walkthrough with connected ESP32-S3 sensor node |
...
| 2 — Hard Scan + Memory | Subham | Delivered producer | FastAPI REST service (GET /api/v1/health, GET /api/v1/objects/locate, POST /api/v1/memory/clear, POST /api/v1/scan), SQLite persistence engine, 2.0s Hard Scan engine, CLI runner, 43/43 tests PASS | Connect trained Locate YOLO artifact when Spandan delivers S09 candidate |
...
| 4 — Android Camera AI | Samik; Spandan artifacts; Rishav shell | Verified on device | CameraXAnalyzer & live PreviewView rendering at 30 FPS on OPPO CPH2753 screen; YOLO perception pipeline bound to lifecycle |
...
| 6 — Fusion + Risk | Samik owns the complete multimodal fusion engine; Rohan supplies sensor records; Rishav consumes decisions in app/voice | Ready for review | Codex implemented the user-authorized Samik-owned reducer: exact source formulas, 300 ms sensor freshness, 25% area growth, maximum-severity fusion, <= 200 ms cautious label association, one-second release/loss/clear holds, source ordering, single camera tracking authority, and FinalSearch isolation. 79 JVM tests pass; APK assembles. Physical/device gates and receiver review remain pending. |
...
| 6 | Samik | Rishav and Rohan | READY | PENDING | Entry 2026-09-15T21:13:50+05:30 at base `1376b2f`; implementation prepared for review with 79/79 JVM tests and debug APK build passing. First 76-test run retained as 2 failed before association expectation correction. No connected phone/sensor; physical AC-05/06/07/09/12 and receiver verdict remain pending. Frozen hashes match after implementation. |
