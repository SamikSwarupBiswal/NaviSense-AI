#!/usr/bin/env python3
"""NaviSense AI — Locate Model Export Utility.

Exports fine-tuned Locate PyTorch weights to:
1. TorchScript (.pt) for laptop
2. PyTorch Mobile Lite (.ptl) for Android LiteModuleLoader

Enforces:
- Output tensor shape: [1, 6, 8400] (for 2 classes: cx, cy, w, h + 2 class scores)
- Updates models/metadata/locate_model_contract.json with SHA-256 hashes
...
logger = logging.getLogger("export_locate_model")
...
def export_locate(
    ...
    smoke_dir: Path = Path("models/smoke"),
    assets_dir: Path = Path("android/app/src/main/assets/models"),
    contract_path: Path = Path("models/metadata/locate_model_contract.json"),
) -> dict:
    ...
    logger.info("=" * 70)
    logger.info("NaviSense Locate Model Export Utility")
    logger.info("=" * 70)
    ...
    ts_path = smoke_dir / "locate_smoke.pt"
    traced_module.save(str(ts_path))
    logger.info(f"Saved TorchScript model to: {ts_path}")

    # Also save to output_dir
    shutil.copy2(ts_path, output_dir / "locate_finetuned.pt")

    # 2. Export to PyTorch Mobile Lite (.ptl)
    ptl_path = smoke_dir / "locate_smoke.ptl"
    logger.info("Exporting to PyTorch Mobile Lite format...")
    try:
        from torch.utils.mobile_optimizer import optimize_for_mobile
        mobile_module = optimize_for_mobile(traced_module)
        mobile_module._save_for_lite_interpreter(str(ptl_path))
    except Exception as e:
        logger.warning(f"optimize_for_mobile unavailable ({e}), using direct _save_for_lite_interpreter")
        traced_module._save_for_lite_interpreter(str(ptl_path))
    logger.info(f"Saved PyTorch Mobile Lite model to: {ptl_path}")

    # Also save to output_dir
    shutil.copy2(ptl_path, output_dir / "locate_finetuned.ptl")

    # 3. Copy to Android assets
    asset_ts = assets_dir / "locate_smoke.pt"
    asset_ptl = assets_dir / "locate_smoke.ptl"
    ...
    if contract_path.exists():
        ...
        contract["provenance"]["status"] = "finetuned_tabletop_phone"
        contract["provenance"]["qualification"] = "QUALIFIED_LOCATE_CANDIDATE"
        contract["provenance"]["note"] = "Fine-tuned on Roboflow tabletop and phone dataset (keys, wallet)."
        ...
        if "artifacts" in contract:
            ...
            if "android_mobile_ptl" in contract["artifacts"]:
                contract["artifacts"]["android_mobile_ptl"]["sha256"] = ptl_hash
            if "android_assets_pt" in contract["artifacts"]:
                contract["artifacts"]["android_assets_pt"]["sha256"] = ts_hash

        with open(contract_path, "w", encoding="utf-8") as f:
            json.dump(contract, f, indent=2)
        logger.info(f"Updated metadata contract at: {contract_path}")
...
def main():
    parser = argparse.ArgumentParser(description="NaviSense Locate Model Exporter")
    parser.add_argument(
        "--weights",
        type=Path,
        default=Path("models/locate/locate_best.pt"),
        help="Input weights (.pt)",
    )
