...
| [Subham](subham/guidance.md) | Laptop Locate, Hard Scan, SQLite, API and Android memory client | In progress | Tabletop dataset delivered (274 images across SES_01, SES_02, SES_03, SES_04; 108 keys, 105 wallets, 48 negatives, plus 81 new raw tabletop images in SES_04); SQLite persistence engine (10/10 PASS), Hard Scan engine (10/10 PASS), FastAPI REST service & runner (11/11 PASS); 43/43 laptop tests pass | Connect trained Locate YOLO artifact when Spandan delivers S09 candidate |
...
| 4 — Android Camera AI | Samik; Spandan artifacts; Rishav shell | In progress | Live PreviewView was previously observed at 30 FPS, but continuation inspection found MainActivity created an unloaded YoloModelRunner with no backend. Code now loads the packaged PyTorch Lite model locally, verifies its hash, and activates it for CameraX; this correction is JVM/build-tested but not yet rerun on a connected phone. |
